...
const runGtfsMatching = (hafasInfo, gtfsInfo, opt = {}) => {
	...
	const matchedItems10s = ttlBuffer({
		ttl: 10000, // 10s
		initialValue: 0,
		in: count => count + 1,
		out: count => count - 1,
	...
	const avgMatchTime = {
		...
		movement: movingAvg(50, 30), // past 100 items, pre-fill with 30ms
		trip: movingAvg(50, 30), // past 100 items, pre-fill with 30ms
	}
	const seenTripIds = {
		all: new Set(),
		movement: new Set(),
		...
		const waitAndMatch = (msg) => {
			...
			const match = async () => {
				...
				if (isMatched) {
					matchedItems.total++
					matchedItems[kind]++
					matchedItems10s.push(1)
					matchedTripIds.all.add(origTripId)
					matchedTripIds[kind].add(origTripId)
				}
				matchedItemsPerSecond = matchedItems10s.valueOf() / 10
			}
