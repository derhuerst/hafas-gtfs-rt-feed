...
const runMonitor = (hafas, opt = {}) => {
	...
	const demandDuration = 'demandDuration' in opt
		? opt.demandDuration
		: (process.env.DEMAND_DURATION
			? parseInt(process.env.DEMAND_DURATION)
			: fetchTripsInterval * 2.5)
	...
	let noDemandTimer = null
	if (onDemand) {
		const onNoDemand = () => {
			logger.debug('pausing monitor because of lack of demand')
			monitor.pause()
		}
		const onDemand = (msg) => {
			clearTimeout(noDemandTimer)
			setTimeout(onNoDemand, demandDuration)

			logger.debug('restarting monitor because of demand')
			monitor.start()
